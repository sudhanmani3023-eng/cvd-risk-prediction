...
from sklearn.metrics import (
    ...
    recall_score, matthews_corrcoef, log_loss, confusion_matrix,
...
def mets(yt, yp, pb):
    return dict(
        AUC     = roc_auc_score(yt, pb[:, 1]),
        CA      = accuracy_score(yt, yp),
...
def mk_cardio(n=70_000):
    ...
    return pd.DataFrame(dict(age=age, gender=gen, height=ht, weight=wt,
        ap_hi=aph, ap_lo=apl, cholesterol=ch, gluc=gl,
        smoke=sm, alco=al, active=ac, target=tgt))

def mk_uci(n=297):
    """Synthesise a balanced Cleveland-like dataset (160 No-CVD, 137 CVD)."""
    ...
    prob    = 1 / (1 + np.exp(-lp))
    # Force exact 160/137 split to match original Cleveland
    thresh  = np.percentile(prob, 160/n*100)
    tgt     = (prob >= thresh).astype(int)
    return pd.DataFrame(dict(age=age, sex=sex, cp=cp, trestbps=tbp,
        chol=cho, fbs=fbs, restecg=rec, thalach=tha, exang=exg,
...
def load_uci():
    ...
    If the local file has wrong target column or severe imbalance,
    fall back to balanced synthesis.
    """
    ...
    if df is not None:
        ...
        # If severely imbalanced (minority < 30%), use synthesis
        vc_check = df["target"].value_counts(normalize=True)
        if vc_check.min() < 0.30:
            w("  [INFO] Local UCI file has severe imbalance "
              f"({vc_check.min()*100:.1f}% minority) — using synthesis")
            df = mk_uci(297)
    else:
        df = mk_uci(297)
...
def sub(X, y, n, seed=SEED):
    if len(y) <= n:
        return X, y
    i, _ = train_test_split(np.arange(len(y)), train_size=n,
...
# ═══════════════════════════════════════════════════════════════
def mlr():
    return LogisticRegression(penalty="l2", C=LR_C,
        class_weight="balanced", solver="lbfgs",
        max_iter=100, random_state=SEED)

def mrf(n=RF_N):
    return RandomForestClassifier(n_estimators=n,
        min_samples_split=RF_MINSPLIT, class_weight="balanced",
        random_state=SEED, n_jobs=-1, max_depth=6)

def mxgb(n_pos=1, n_neg=1, n=XGB_N):
    return xgb.XGBClassifier(n_estimators=n, eval_metric="logloss",
        random_state=SEED,
...
def holdout(df, tag):
    ...
    w(f"Train  : {ntr:,} ({ntr/nt*100:.1f}%)  "
      f"CVD={c1tr:,}  NoCVD={c0tr:,}")
    w(f"Test   : {nte:,} ({nte/nt*100:.1f}%)  "
      f"CVD={c1te:,}  NoCVD={c0te:,}")
...
def do_cv(proto, X_tr, y_tr, name, tag, ntr_rep, nval_rep):
    ...
    w("─" * 48)
    w(f"{'Mean':>4}  {'':>7}  {'':>6}  "
      f"{np.mean(accs):>7.4f}  {np.mean(f1s):>7.4f}  "
      f"{np.mean(aucs):>7.4f}")
    w(f"{'Std':>4}  {'':>7}  {'':>6}  "
      f"{np.std(accs):>7.4f}  {np.std(f1s):>7.4f}  "
      f"{np.std(aucs):>7.4f}")
...
def do_repeat(proto, X, y, name, tag, ntr_rep, nte_rep):
    ...
    # plot
    mk = list(mets(np.array([0, 1]), np.array([0, 1]),
                   np.array([[1., 0.], [0., 1.]])).keys())
    fig, axes = plt.subplots(2, 4, figsize=(12, 5))
    axes = axes.flatten()
    for i, mn in enumerate(mk):
        v = arr[:, 2 + i]
    ...
    plt.tight_layout()
    path = (f"outputs/repeat_"
            f"{name.lower().replace(' ', '_')}_{tag}.png")
    plt.savefig(path, dpi=DPI); plt.close()
    w(f"[Saved] {path}")
...
def evalu(model, X_te, y_te, name, tag):
    ...
    prec = mt["Prec"]; rec = mt["Recall"]; mcc = mt["MCC"]
    spec = mt["Spec"]; ll  = mt["LogLoss"]
    ...
    w(f"TP={tp:,}  TN={tn:,}  FP={fp:,}  FN={fn:,}  "
      f"Total={tp+tn+fp+fn:,}")
    preds(y_te, yp, pb, name, tag)
    # CM image
    fig, ax = plt.subplots(figsize=(4, 3))
    sns.heatmap(cm_e, annot=True, fmt="d", cmap="Blues", ax=ax,
    ...
    cpath = (f"outputs/cm_"
             f"{name.lower().replace(' ', '_')}_{tag}.png")
    plt.savefig(cpath, dpi=DPI); plt.close()
    w(f"[Saved] {cpath}")
    return mt, yp, pb, cm_e

def preds(y_te, yp, pb, name, tag):
    n   = len(y_te)
...
def plots(results, mdict, X_te, y_te, tag):
    ...
    for i, mn in enumerate(met):
        ax.bar(xx + i*ww,
               [results[n][mn] for n in mnames], ww,
               label=mn,
               color=["steelblue","darkorange",
                      "forestgreen","crimson"][i])
...
# ═══════════════════════════════════════════════════════════════
# SHAP — auto-select explainer by model type
# ═══════════════════════════════════════════════════════════════
def do_shap(model, X_te, y_te, tag, name, feat):
    ...
    try:
        if isinstance(model, LogisticRegression):
            ...
            # sv shape: (n, features) for binary — use as-is
